 ...
 */
 ...
 if ( ! class_exists( 'Law_Firm_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Law_Firm_WebFont_Loader {
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'law_firm_get_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'law_firm_get_local_fonts_subfolder_name', 'fonts' );
			}
...
if ( ! function_exists( 'law_firm_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function law_firm_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Law_Firm_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'law_firm_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function law_firm_get_webfont_url( $url, $format = 'woff2' ) {
		$font = new Law_Firm_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
