<?php
/**
 * Law Firm Pro Customizer Repeater Control.
 * 
 * @package Law_Firm
*/
...
if( ! class_exists( 'Law_Firm_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Law_Firm_Control_Repeater extends WP_Customize_Control {
    	public $type = 'law-firm-repeater';
    	...
    	public function __construct( $manager, $id, $args = array() ) {
    		...
    		if ( empty( $this->button_label ) ) {
    			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'law-firm' ), $this->row_label['value'] );
    		}
    	...
    	public function enqueue() {
            wp_enqueue_script( 'law-firm-repeater', get_template_directory_uri() . '/inc/customizer/customizer-controls/repeater/assets/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
    		wp_enqueue_style( 'law-firm-repeater', get_template_directory_uri() . '/inc/customizer/customizer-controls/repeater/assets/repeater.css', null );
    	}
    	...
    	protected function render_content() {
    		...
    		<?php if ( isset( $this->choices['limit'] ) ) : ?>
    			<p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'law-firm' ), esc_html( $this->choices['limit'] ) ); ?></p>
    		<?php endif; ?>
    		...
    		<script type="text/html" class="customize-control-repeater-content">
    			...
    			<li class="repeater-row minimized" data-row="{{{ index }}}">
    				...
    				<div class="repeater-row-content">
    					<# _.each( data, function( field, i ) { #>
    						...
    						<div class="repeater-field repeater-field-{{{ field.type }}}">
    							...
    							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
    								...
    								<figure class="law-firm-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'law-firm' ); ?>" >
    									<# if ( field.default ) { #>
    										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
    										<img src="{{{ defaultImageURL }}}">
    									<# } else { #>
    										<?php esc_attr_e( 'No Image Selected', 'law-firm' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'law-firm' ); ?></button>
    									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'law-firm' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'law-firm' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_attr_e( 'Change Image', 'law-firm' ); ?>
    										<# } else { #>
    											<?php esc_attr_e( 'Add Image', 'law-firm' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    							...
    							<# } else if ( field.type === 'upload' ) { #>
    								...
    								<figure class="law-firm-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'law-firm' ); ?>" >
    									<# if ( field.default ) { #>
    									...
    									<# } else { #>
    										<?php esc_attr_e( 'No File Selected', 'law-firm' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
    									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'law-firm' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'law-firm' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_attr_e( 'Change File', 'law-firm' ); ?>
    										<# } else { #>
    											<?php esc_attr_e( 'Add File', 'law-firm' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
    					...
    					<# }); #>
    					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'law-firm' ); ?></button>
    				</div>
    			</li>
    		</script>
    		<?php
